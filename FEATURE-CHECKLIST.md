...
- **Payment gateway integration (Paystack)** – Proceed to payment from reserve form; success and cancel return pages
...
- **Testing** – E2E (e.g. Playwright) for critical paths: login → browse → reserve (up to Paystack checkout), payment return pages
...
- [x] Route rules: ISR for `/`, `/documents`
...
- [x] `/reserve/[unitNumber]` – Reservation form (lock, timer, Paystack checkout)
...
- [x] `/payment-success` – Post-Paystack success page
- [x] `/payment-cancel` – Post-Paystack cancel page (and one-time toast on return to home)
...
- [x] Reserve form: contact details, terms, Paystack checkout
...
- **Done:** Auth, layout, **create-lead on signup (Zoho)**, all main pages (including payment-success and payment-cancel), units list & detail, filters, reserve form, lock timer, wishlist, reservations list, documents page, server APIs for lock/viewers/time, **Paystack checkout flow and return pages.**
